...
- **Args CLI**: `--carpeta`, `--ventana-temporal`, `--hash-umbral`, `--umbral-tiempo-segundos`, `--criterio` (calidad|tema|diversidad|descripcion|nitidez), `--modelo` (selección), `--modelo-clustering` (agrupación, default moondream), `--modelo-embed` (embeddings, default nomic-embed-text), `--criterio-agrupacion` (embeddings|tiempo|phash|tags), `--no-proxy`, `--no-similitud`, `--carpeta-excluir`, `--dry-run`, `--json`
...
- **Notas**: 
  ...
  - 3 modelos independientes: selección (minicpm), clustering (moondream), embeddings (nomic). Los prompts de clustering van EN (moondream responde mal en español).
