...
> ```
> El modelo CED-mini (527 clases AudioSet, int8, ~100 MB) se **descarga solo**
> la primera vez que se ejecuta `audio_tagging.py` (desde GitHub Releases) y se
> extrae en `models/audio/sherpa-onnx-ced-mini-audio-tagging-2024-04-19/`.
> La URL oficial del asset (si se quiere bajar a mano):
> ```
...
> La descarga automática se puede deshabilitar con `--no-descargar`.
...
│   ├── improve_db.py                # Post-procesamiento (10 pasos, ~1350 lines)
...
- **Propósito**: Pipeline de 10 pasos post-ingesta con skip/update/replace.
- **Args CLI**: `--steps` (default: todos), `--mode` (skip|update|replace), `--db`, `--list`, `--no-mostrar` (invertido: por default muestra en vivo keywords/descripciones), `--workers` (default 1)
- **Pasos**: `colors`, `keywords`, `descriptions`, `combinado`, `transcribe`, `keypoints`, `timestamps`, `gps`, `video_metadata`
- **DB que modifica**: `media` (UPDATE colores, timestamps, GPS), `media_metadata` (INSERT keywords, descriptions, transcripts, `whisper_segments`/`whisper_info`/`whisper_estado`, claves `*_en`), `media_keypoints`
- **Modos**: skip (solo pendientes), update (actualiza todos), replace (limpia y regenera)
- **Dependencias**: color_utils, ai_media/image_analysis, ai_media/transcribe_media, ai_media/traducir_metadata, ai_media/transcribe (faster-whisper)
- **TRANSCRIBE (`transcribe`)**: usa `faster-whisper` con **`vad_filter=True`** (Silero VAD: detecta persona hablando, descarta ruido/silencio ambiental, devuelve 0 segmentos en clips sin voz), **autodetección de idioma** (`language=None`, NO fuerza español — alguien podría hablar otro idioma), **`condition_on_previous_text=False`** (corta lazos de repetición: "I'm going to finish it" ×10), y umbrales de confianza `no_speech_threshold=0.6`, `compression_ratio_threshold=2.4`, `log_prob_threshold=-0.8`. Modelo `small`. Guarda **métricas por segmento** (`promedio_logprob`, `no_hay_habla_prob`, `ratio_compresion`) y escribe `whisper_estado` (ok | sin_voz). **Solo guarda `whisper_segments`/`whisper_info` si el estado es `ok`** (en `sin_voz` no acumula basura: queda únicamente la marca `whisper_estado`). Usa **checkpoint incremental** (`Checkpoint` cada 20, no commit único final). **El modo `skip` es auto-recuperable**: el marcador de "terminado" es `whisper_estado` — un archivo se considera pendiente si NO tiene `whisper_estado` (ni tocado, ni corte a mitad de batch/checkpoint); así cualquier corrida interrumpida se retoma en la siguiente pasada. No se usa `whisper_segments` para detectar pendientes porque un `sin_voz` no lo tiene (se re-transcribiría cada vez).
- **FLUJO IA (keywords/descriptions/combinado)**: 2 fases por paso. **Fase A (visión)**: minicpm genera EN y lo guarda en `ia_keywords_en`/`ia_description_en`. **Fase B (traducción)**: translategemma traduce a ES sobre la DB (sin re-procesar imágenes) y escribe `ia_keywords`/`ia_description` (ES definitivo, lo que consume la interfaz). Al regenerar el EN SIEMPRE se invalida el ES viejo (incluido skip) para que la fase B lo retraduzca. El EN queda persistido para re-traducir sin re-correr visión. (Modelo: translategemma, 3.3GB 4.3B, especializado en traducción; elegido sobre qwen2.5:3b tras auditoría que detectó chino `户外`, checklists `ripio/grava` y checklist hinchada generados por el traductor viejo. Sin glosario: era decorativo para un traductor entrenado y las reglas empujaban al modelo viejo a checklist/slash.)
...
#### `limpiar_tandas.py`
- **Propósito**: Limpieza de tandas/bursts de fotos. Agrupa por tiempo, sub-agrupa por criterio semántico y elige la mejor de cada grupo; mueve descartados a carpeta `excluir/`.
- **Args CLI**: `--carpeta`, `--ventana-temporal`, `--hash-umbral`, `--umbral-tiempo-segundos`, `--criterio` (calidad|tema|diversidad|descripcion|nitidez), `--modelo` (selección, default moondream), `--modelo-clustering` (agrupación, default moondream), `--modelo-embed` (embeddings, default nomic-embed-text), `--criterio-agrupacion` (embeddings|tiempo|phash|tags), `--no-proxy`, `--no-similitud`, `--carpeta-excluir`, `--limpiar-proxies`, `--dry-run`, `--json`
...
- **Notas**: 
  ...
  - `--criterio nitidez` elimina la pasada de visión para seleccionar (instantáneo, sin IA).
  - 3 modelos independientes: selección (`MODELO_SELECCION_DEFAULT=moondream:latest`), clustering (moondream), embeddings (nomic). La limpieza es solo curación (no escribe en la DB), por eso la selección usa moondream (rápido, ~0.8s/img) y no el modelo del FLUJO IA (minicpm). Los prompts de clustering Y de selección van EN (moondream responde mal en español).
  - **Limpieza de proxies**: al mover una imagen descartada se llama `limpiar_proxies(ruta)` para borrar su proxy de 800px (el `.proxies/` queda en la carpeta original, no sigue a la imagen, y `excluir/` queda fuera del escaneo). `--limpiar-proxies` borra toda la carpeta `.proxies/` de la raíz al terminar (no rompe el caché de las conservadas durante la corrida).
...
- **2 capas de refinamiento**:
  ...
  2. **Diccionario de sinónimos**: `SINONIMOS` (términos canónicos del dominio: bicicleta, motocicleta, automóvil, ruta, montaña, ciclismo, etc.) + `GENEROS_FOTOGRAFICOS` con `VARIANTES_GENERO` (nocturno→nocturna, street→callejera, etc.)
- **⚠️ Capa semántica ELIMINADA (Ago 2026)**: se usaba `--usar-embeddings` con `paraphrase-multilingual:latest` para agrupar sinónimos. Tras re-traducir con translategemma se detectó que los embeddings introducían **falsos sinónimos** que degradaban términos específicos del dominio (`ciclismo`→`deporte`, `nublado`→`soleado`, `parche`→`parque`, `cesta`→`ruta`). Como translategemma ya produce keywords limpias y consistentes, la unificación por embeddings no aportaba y podía meter errores → se ELIMINÓ por completo (no queda ni como opción). También se corrigió el diccionario para que `ciclismo` y `ciclista(s)` NO colapsen a `deporte`/`personas`: `ciclismo` pasó a término canónico propio (variantes: ciclista, ciclistas, cycling, cyclist, cyclists, pedaleando).
...
| `transcribe.py` | Transcripción vía faster-whisper (independiente, sin DB). Formatos: SRT, TXT, JSON. Soporta **VAD** (`vad_filter`/`vad_parameters`), `condition_on_previous_text`, umbrales de confianza y **métricas por segmento** (`promedio_logprob`, `no_hay_habla_prob`, `ratio_compresion`). Helpers: `filtrar_segmentos_confiables()` (logprob≥-0.8, no_habla<0.6, compresión<2.4, duración≥1.5s), `clasificar_estado()` (ok\|sin_voz). | faster-whisper |
...
| `batch_selector.py` | Selecciona la mejor imagen de una tanda. Modelo por defecto `MODELO_SELECCION_DEFAULT=moondream:latest` (solo curación, no escribe en la DB ~0.8s/img). ⚠️ prompts de selección **EN** (moondream responde mal en español). Criterios: calidad/tema/diversidad/descripcion (Ollama) + nitidez (varianza Laplaciano, sin IA). | ollama_client |
| `clustering.py` | Agrupa imágenes por tags o embeddings compartidos. Modelo por defecto `MODELO_CLUSTERING_DEFAULT=moondream:latest` (rápido, ~0.8s/img; la descripción NO se reutiliza en la DB). ⚠️ prompts **EN** (moondream responde mal en español). `modelo_embed` para el embedding (default nomic-embed-text). | ollama_client |
| `generate_embeddings.py` | Genera embeddings vectoriales vía nomic-embed-text. Texto **multi-fuente**: transcripción por segmentos (`whisper_segments` JSON o keypoints `transcription` ordenados por offset), `ia_description`, `ia_keywords`, `ia_keywords_transcripcion`, `ia_keywords_sonido` y caption de Telegram vinculado. Límite MAX_TEXTO_CHARS=6000 (corte sin partir palabras). Soporta `--mode skip|update|replace`. Antes solo leía `ia_description`/`transcript` (clave que NO existe en la DB → los audio/video jamás se embebían). | ollama_client |
| `checkpoint.py` | Helper de **checkpoint y detención limpia** para procesos IA lentos. `Checkpoint(conn, cada=20)` → `.contar()` commitea la DB cada 20 ítems y `.finalizar()` hace commit final (progreso preservado ante Ctrl+C/cuelgue). `manejar_interrupcion(conn=...)` (context manager) captura `KeyboardInterrupt`, commitea pendientes, imprime mensaje amigable y termina con `sys.exit(130)` sin traceback. Usado por improve_db, refinar_keywords, traducir_metadata, keywords_transcripciones, audio_tagging. | — |
| `refinar_keywords.py` | Refina y unifica keywords de IA (2 capas: léxica, diccionario de sinónimos). Sin capa semántica desde Ago 2026 (los embeddings de paraphrase eran inestables e introducían falsos sinónimos). Sobrescribe `media_metadata.ia_keywords`. | — |
| `traducir_metadata.py` | Traduce metadata EN → ES sobre la DB (keywords/descripciones) con **translategemma** y modo JSON combinado. Guarda `ia_keywords`/`ia_description` (ES) desde `ia_keywords_en`/`ia_description_en`. Sin glosario desde Ago 2026 (era decorativo para un traductor entrenado y sus reglas empujaban al modelo viejo a checklist/slash). | ollama (texto) |
...
3. MEJORAR DB: python flujos.py improve-db [--mode skip|update|replace]
   ...
   ├── 4. transcribe   → ai_media/transcribe.py (faster-whisper + VAD + confianza)
...
- **Optimización limpiar_tandas (Ago 2026)**: 3 cambios para acelerar la limpieza de tandas (que es el paso 1 del pipeline, antes de ingestar): (1) **proxy al fin efectivo** — `clustering.py` y `batch_selector.py` pasaban las imágenes ORIGINALES a la IA; `obtener_proxy` se importaba pero nunca se usaba en ese flujo (el flag `--no-proxy` era decorativo). Ahora usan proxy de 800px en TODAS las llamadas de visión (tags/embeddings + calidad/tema/diversidad/descripcion), ~2.5x más rápido. (2) **Nuevo criterio de selección `nitidez`** en `batch_selector.py` (`_seleccionar_por_nitidez`): varianza del Laplaciano con Pillow/ImageStat, SIN IA (~ms). Elimina la 2da pasada de visión por imagen para seleccionar la mejor de cada sub-grupo. (3) **Modelo de clustering separado**: `MODELO_CLUSTERING_DEFAULT=moondream:latest` en `clustering.py` (era minicpm, rama del FLUJO IA). Benchmark con 7 fotos reales: moondream ~0.8s/img vs minicpm ~3-13s/img, con agrupación comparable. La descripción del clustering (extraída SOLO para el embedding de agrupación) NO se reutiliza en la DB ni se guarda en sidecar. Coherente con esto, `limpiar_tandas` expone 3 modelos independientes: `--modelo` (selección, default minicpm), `--modelo-clustering` (agrupación, default moondream), `--modelo-embed` (embeddings, default nomic-embed-text, NUEVO — antes era fijo). ⚠️ **moondream responde MAL a prompts en español** (regurgita basura tipo "irtiville"); los prompts de `clustering.py` (descripción y tags) se pasaron a **inglés**. Modelos vision con `num_ctx=4096` fijo: `granite3.2-vision` excede el contexto (5179 tokens) y no sirve sin subir `num_ctx`. Bug fix: `imprimir_reporte` usaba `≥` (U+2265) que rompe con el codec `charmap` de consolas Windows (error "charmap codec can't encode character") → ahora `>=`.
- **Limpieza de tandas — selección moondream (Ago 2026)**: se desacopló la **selección** de la mejor imagen del default del FLUJO IA (minicpm) creando `MODELO_SELECCION_DEFAULT=moondream:latest` en `batch_selector.py` (default en `seleccionar_mejor_imagen()`, `seleccionar_mejores_n()` y el CLI `--modelo`; `limpiar_tandas.py` importa y usa esa constante). La selección es solo curación (no escribe en la DB), así que no hace falta el español ni el modelo pesado. El FLUJO IA (image_analysis, improve_db, tag_images) **NO se tocó**: sigue con minicpm donde sí importa calidad y español. Además, los prompts de selección (`PROMPT_EVALUAR_CALIDAD` y el de `_seleccionar_por_tema`) se pasaron a **inglés escuetos** (moondream responde mal en español). Criterios `descripcion`/`diversidad` ya delegaban en prompts EN de image_analysis, sin cambios.
- **Limpieza de proxies en limpiar_tandas (Ago 2026)**: `limpiar_todos_los_proxies` se importaba pero era un **import muerto** — cada corrida acumulaba proxies de 800px en `.proxies/` y al mover las descartadas a `excluir/` sus proxies quedaban huérfanos en la carpeta original. Ahora `_mover_a_excluir()` llama `limpiar_proxies(ruta)` por cada descartada (borra proxy + hash), y se agregó `--limpiar-proxies` para borrar toda la carpeta `.proxies/` de la raíz al terminar.
- **Timezone en día de la semana y loop (Ago 2026)**: `loop_db.py._extraer_hora()` usaba la hora UTC directa (el loop quedaba 3h tarde en Argentina) y `dia_semana.py` calculaba el día en UTC (23:30 local del lunes → "martes"). Fix: `_extraer_hora()` ahora convierte a Argentina (`timezone(timedelta(hours=-3))`) antes de sacar la hora; `dia_semana.parsear_timestamp()` normaliza `Z` y convierte a Argentina antes de `weekday()`. Ambos con guardián `if dt.tzinfo is not None` para no depender de la zona del SO sobre naive.
- **Timezone robustez improve_db + fetch_weather (Ago 2026)**: en `improve_db.py` la interpolación de timestamps (`run_timestamps`) parseaba con `fromisoformat` directo y el `astimezone(-3)` posterior podía usar la zona del SO si el timestamp quedó naive → ahora se fuerza aware UTC con `_as_aware_utc()` (normaliza `Z`, `replace(tzinfo=utc)` si naive). También se normalizó `Z` en `run_keypoints` (775) y `run_gps` (945-947). En `fetch_weather.py` se normalizó `Z` en las 3 apariciones de `fromisoformat` (181, 255, 403) para robustez (Python ≥3.11 ya acepta `Z`, pero las bases viejas podrían tener el formato mixto).
- **Auditoría de keywords y cambio de traductor a translategemma (Ago 2026)**: tras auditoría cruzada EN vs ES se confirmó que la basura (`户外`, `ripio/grava`, checklist de `banquina/manubrio/...`) la generaba el **traductor qwen2.5:3b** (colapso en checklist/chino), NO la visión (minicpm genera EN limpio). Batería de 11 casos reales de la DB: qwen2.5:3b score −2.3 (3 casos chino, 5 checklist, 5 slash) vs **translategemma +1.6** (0/0/0, traducciones fieles, conteo de tokens exacto 10/11). Como los grandes (llama3.1 4.9GB, qwen3.5 6.6GB) no ofrecen ventaja y pesan más (requisito: correr en hardware limitado), se eligió translategemma (3.3GB, 4.3B, especializado en traducción). Cambios: `MODELO_TRADUCCION_DEFAULT = "translategemma"` en `traducir_metadata.py` y el default de `_traducir_metadata` en `improve_db.py`. Se re-tradujeron los 702 registros (`--paso keywords --mode update`): **702 ok, 0 errores, 0.70s/img**; verificado en DB: chino 15+ → 0, slash 55 → 1, checklist 90 → 1. Test A/B del GLOSARIO demostró que era **decorativo** (0 usos en 11 casos con/sin) y su regla "usá EXACTAMENTE" empujaba al modelo viejo a checklist/slash → se ELIMINÓ de `traducir_metadata.py`. El pipeline de `refinar_keywords.py` quedó en 2 capas (léxica + diccionario) y `ciclismo`/`ciclista(s)` dejaron de colapsar a `deporte`/`personas` (ciclismo pasó a término canónico propio).
- **Capa semántica de refinar_keywords ELIMINADA (Ago 2026)**: `--usar-embeddings` con `paraphrase-multilingual:latest` producía **falsos sinónimos** que degradaban el dominio: `ciclismo→deporte`, `nublado→soleado`, `parche→parque`, `cesta→ruta`, `barra→barro`, `equipo→posada`. Con translategemma las keywords ya salen limpias y consistentes, así que la unificación por embeddings no aportaba y podía meter errores → se ELIMINÓ por completo (función `refinar_con_embeddings`, `similitud_coseno`, `MODELO_EMBEDDINGS`, args `--usar-embeddings`/`--umbral` e import `math`). Se documentó el motivo en el docstring del script y se quitó la integración del TUI (opción 7 ahora llama sin `--usar-embeddings`). Extra: el endpoint de embeddings de Ollama falla con batches >~300-400 (`/tokenize` 400); quedó documentado para quien quiera reintentar embeddings con lotes ≤200.

- **`--mostrar` → `--no-mostrar` en improve_db (Ago 2026)**: el flag de mostrar en vivo keywords/descripciones por imagen quedó **invertido**: ahora se muestra por default y `--no-mostrar` lo silencia. Aplica a los pasos keywords, descriptions y combinado (texto EN de generación). Se actualizaron docstring, epílogo y `CONTEXTO["mostrar"]=True`.
- **Checkpoint y detención limpia en procesos IA (Ago 2026)**: nuevo helper `scripts/ai_media/checkpoint.py` con `Checkpoint(conn, cada=20)` (commitea la DB cada 20 ítems — antes `improve_db.py` acumulaba todo en memoria y commiteaba UNA vez al final del pool, perdiendo el progreso ante Ctrl+C/cuelgue) y `manejar_interrupcion(conn=...)` (context manager que captura `KeyboardInterrupt`, commitea pendientes, imprime mensaje amigable y sale con `sys.exit(130)` sin traceback). Aplicado en: `improve_db.py` (`_procesar_vision`, `run_combinado`, `_traducir_metadata` — ahora también con `as_completed(futures, timeout=timeout_future)` (300s configurable en `CONTEXTO`) y `pool.shutdown(wait=False, cancel_futures=True)` en timeout/interrupción, resolviendo el **riesgo alto documentado** de worker colgado que congelaba el pool para siempre), `refinar_keywords.py` (antes un solo commit final), y captura de Ctrl+C en `traducir_metadata.py`, `keywords_transcripciones.py`, `audio_tagging.py` (ya commiteaban por ítem). El patrón de los mains que no tenían manejo: dividir el cuerpo en `_ejecutar(...)` y envolverlo con `with manejar_interrupcion(...)`. Verificado con test del helper (45 ítems → 3 commits parciales, interrupción → SystemExit(130) con pendiente commiteado).
- **Ampliación multi-fuente de embeddings (Ago 2026)**: `generate_embeddings.py` reescrito: `obtener_texto_a_embeder()` ahora construye texto desde 6 fuentes (transcripción por segmentos `whisper_segments`/keypoints `transcription`, `ia_description`, `ia_keywords`, `ia_keywords_transcripcion`, `ia_keywords_sonido`, caption de Telegram vinculado) con `MAX_TEXTO_CHARS=6000` y corte sin partir palabras. **Causa raíz**: la clave `transcript` NO existía en la DB (las transcripciones viven como segmentos), por lo que los audio/video jamás se embebieron. Nuevo flag `--mode skip|update|replace`. Verificado: 514 candidatos sin embedding → 291 ahora construibles (17/20 en una muestra), 3 embeddings reales generados OK, 817 totales en DB.
- **Transcripción con VAD + filtro de alucinaciones (Ago 2026)**: auditoría de las 217 transcripciones detectó **alucinaciones masivas de Whisper**: solo 144/217 (66%) se detectaron como español; 73 en idiomas aleatorios (noruego-nynorsk 28, inglés 36, italiano 4, javanés 2, coreano 1, portugués 1, turco 1) y de esas 58 con confianza de idioma < 0.5. Causa raíz: `run_transcribe` corría faster-whisper sin VAD, con `language=None` y `condition_on_previous_text=True`, sobre clips de **ruido ambiental (cámaras GoPro sin habla)** → inventaba basura repetitiva ("I'm going to finish it" x10, "Bu ne? Bu ne?"), incluso con `language_probability` alta (0.79 → "4-5-6-7-8"). **Fixes**: (1) `transcribe.py` gana soporte VAD (`vad_filter=True`, `vad_parameters={'min_speech_duration_ms':300}`), `condition_on_previous_text=False`, umbrales (`no_speech_threshold=0.6`, `compression_ratio_threshold=2.4`, `log_prob_threshold=-0.8`), métricas por segmento (`promedio_logprob`/`no_hay_habla_prob`/`ratio_compresion`) y helpers `filtrar_segmentos_confiables()`/`clasificar_estado()` (ok|sin_voz → si Whisper no lo oyó, un humano tampoco; es audio de mala calidad que no vale la pena). (2) `improve_db.py`: `run_transcribe` pasa a modelo `small` + VAD + **autodetección de idioma** (NO se fuerza español — alguien podría hablar otro idioma) + filtro de confianza + checkpoint incremental y escribe `whisper_estado`. `run_transcribe` **no guarda basura**: solo persiste `whisper_segments`/`whisper_info` si clasifica `ok`, quedando únicamente `whisper_estado=sin_voz` en el ruido. El modo `skip` es **auto-recuperable**: el marcador de "terminado" es `whisper_estado` (no `whisper_segments`, porque un `sin_voz` queda con estado pero sin segmentos), así cualquier corrida interrumpida se retoma en la siguiente pasada. Verificado: filtros (buena→ok, alucinación/no_habla/blip/logprob-bajo→sin_voz), faster-whisper 1.2.1 acepta los parámetros, y prueba real con `audio_78` (antes "Jamehni gira si anyu", ahora 0 segmentos → sin_voz).
